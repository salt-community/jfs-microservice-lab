...
class InventoryControllerTest {
  ...
  void testGetAllInventoryItems_emptyList() throws Exception {
    ...
    // Act & Assert
    mockMvc
        .perform(get("/api/inventory").contentType(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.length()").value(0));
  }

  // Test to check if a new inventory item can be created successfully and returns a 201 Created
  // status
  @Test
  ...
  void testGetInventoryItemById_found() throws Exception {
    ...
    // Act & Assert
    mockMvc
        .perform(get("/api/inventory/{id}", item.getId()).contentType(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.product").value("Existing Product"));
  }

  // Test for retrieving an inventory item by ID when the item is not found, ensuring a 404 Not
  // Found status
  @Test
  ...
  void testDeleteInventoryItem() throws Exception {
    ...
    // Act & Assert
    mockMvc
        .perform(delete("/api/inventory/{id}", UUID.randomUUID()))
  ...
  void testUpdateInventoryItem() throws Exception {
    ...
    mockMvc
        .perform(
            put("/api/inventory/{id}", updatedItem.getId())
                .contentType(MediaType.APPLICATION_JSON)
                .content(
