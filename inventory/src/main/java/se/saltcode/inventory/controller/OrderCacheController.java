...
public class OrderCacheController {
  ...
  public ResponseEntity<List<OrderCacheDto>> getOrderCache() {
    return  ResponseEntity.ok(orderCacheService.getAllOrderCacheItems().stream().map(OrderCache::toDto).toList());
  ...
  @PostMapping
  public ResponseEntity<OrderCacheDto> createOrderCacheItem(@RequestBody AddOrderCacheDto addOrderCacheDto) {
    OrderCacheDto orderCacheDto = orderCacheService.createOrderCacheItem(new OrderCache(addOrderCacheDto)).toDto();
    return ResponseEntity.created(URI.create(apiUri + "/" + orderCacheDto.id())).body(orderCacheDto);
  }
